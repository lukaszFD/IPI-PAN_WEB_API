...
namespace DB_ModelEFCore.Controllers.Audit
{
    public class AuditData
    {
        ...
        /// <returns></returns>
        public async Task<IEnumerable<AuditAccounts>> AuditAccounts(string userName, int pageSize, int pageNumber)
        {
            var query = (from 
                        a in _audit.Set<AudAccounts>().Where(a=> a.UserName == userName)
                        join
        ...
        public async Task<IEnumerable<AuditServers>> AuditServers(string serverExId, int pageSize, int pageNumber)
        {
            var query =(from s in _audit.Set<AudServers>().Where(s => s.ExternalId == new Guid(serverExId))
                        join
                        c1 in _repo.Set<RepoCountryRegion>().ToList() on s.NewCountryId equals c1.CountryId
                        join
                        c2 in _repo.Set<RepoCountryRegion>().ToList() on s.OldCountryId equals c2.CountryId
